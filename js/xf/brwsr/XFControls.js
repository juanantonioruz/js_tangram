    ...
    if (tag == "script") {
        ...
        if (evt != null && evt != "") {
            ...
            if (this.__parent.constructor === XFAction) {
                this.__evtCtx = xpCtx;
                if(window.opener == null){/*SH.print("opener is null?");*/};
...
function __xfHtmlFireEvent(evt) {
    //if(!listener.__nonprop)
    if(this.__xfObj && this.__xfObj.__form)	this.__xfObj.__form.fireEvent(evt.type, this.__xfObj, null, {uiEvent:evt}); //ELPM
}
...
XFCtrlBase.prototype.setReadonly = function(bool, initMode) {
    ...
    if (this.__hGrid != null){
        if(this.__hGrid.rows){
            for(var i = 0; i < this.__hGrid.rows.length; i++){
                if(this.__hGrid.rows[i].children[0] && this.__hGrid.rows[i].children[0].children[0]) this.__hGrid.rows[i].children[0].children[0].disabled = bool;
            };
        };
    } else if (this.__hWidget != null) {
...
function XFRepeat(form, elem, pObj) {
    ...
    this.__initBind(form, elem, pObj);
    this.__initCtrl();
    this.isMonster = elem.getAttribute("monster") ? elem.getAttribute("monster") : false;
    var id = this.__id;
    if (id) {
...
XFRepeat.prototype.render = function(xpCtx, hParent, hBefore) {
    ...
    if(SH.is_ie){  //IDEATE
	 //alert("wanna try something else...");
	//setTimeout("alert('0 seconds')", 0);
    	this.__renderedNs = this.getNodeSet(xpCtx);
	//alert("?");
    	//this.__renderedNs = setTimeout("this.getNodeSet(xpCtx);", 10000);
    }else{
	this.__renderedNs = this.getNodeSet(xpCtx);
    }
    ...
    if (!ctrl.noEdit) {
        ...
        setEventHandler(calElem, "onclick",
            function (evt) {
                ...
                if(!SH.is_ie) ctrl._fmtDateElem.focus(); //ELPM
            });
 ...
 * 
 */
XFIxWeave.prototype = new XFCtrlBase();
XFIxWeave.prototype.constructor = XFIxWeave;

function XFIxWeave(form, elem, pObj) {
    this.__initEvt(form, elem, pObj);
    this.__bindRequired = false;
    this.__initBind(form, elem, pObj);
    this.__exfIf = Utilities.getAttributeNS(
...
XFIxWeave.prototype.render = function(xpCtx, hParent, hBefore) {
    this.__xpCtx = xpCtx;
    if (__XFActUtil.__exfIf(this)) {
    	this.__hElem = makeElement(hParent, "div", this.__elem.getAttribute("class"), null, hBefore);
    	...
    	var frmConfig = "";
    	var dataSrc = "";
	var frmName = this.__form.__name + this.__elem.getAttribute("id");           
           if(frmSrc.substr(0, 1) == "{" && frmSrc.substr(frmSrc.length - 1, 1) == "}"){
               frmSrc = this.__model.getValue("instance('form')/" + frmSrc.substr(1, frmSrc.length - 2));
           };
	var frmUri = this.__form.resolveUri(frmSrc);
	this.__sfrm = new XForm(xb.loadURI(frmUri), frmName, xb, frmUri, this.__form.__context);
            this.__sfrm.__models.fmodel.parentModel = this.__model;
...
XFIxWeave.prototype.dispose = function() {
    if (this.__sfrm != null) {
        this.__sfrm.dispose();
        this.__sfrm = null;
    }
...
XFIxSubform.prototype.render = function(xpCtx, hParent, hBefore) {
	...
	var frmName = this.__form.__name + frmSrc;
		//Additions for dynamic subform embedding
		if(frmSrc == "{dashboard}"){
			frmSrc = this.__model.__dashboardURI;
			frmName = this.__form.__name + this.__elem.getAttribute("id");		
		}else if(frmSrc == "{switchControl}"){
			frmSrc = "/system/resources/services/formSwitch/switchController.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			frmConfig = dataSrc; dataSrc = "";
		}else if(frmSrc == "{switchContainer}"){
			frmSrc = "/system/resources/services/formSwitch/switchContainer.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_container";
			frmConfig = dataSrc; dataSrc = "";
		}else if(frmSrc == "{switchControlDynamic}"){
			frmSrc = "/system/resources/services/droolsPoweredTabSet/switchController.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			frmConfig = dataSrc; dataSrc = "";
		}else if(frmSrc == "{switchContainerDynamic}"){
			frmSrc = "/system/resources/services/droolsPoweredTabSet/switchContainer.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_container";
			frmConfig = dataSrc; dataSrc = "";
		}else if(frmSrc == "{tabControl}"){
			frmSrc = "/system/resources/services/appTabSet/switchController.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			dataSrc = "";
		}else if(frmSrc == "{tabControlErr}"){
			frmSrc = "/system/resources/services/appTabSet/switchControllerErr.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			dataSrc = "";
		}else if(frmSrc == "{tabControlFECErr}"){
			frmSrc = "/system/resources/services/appTabSet/switchControllerFECErr.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			dataSrc = "";
		}else if(frmSrc == "{tabControlDyn}"){
			frmSrc = "/system/resources/services/appTabSet/switchControllerDyn.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			dataSrc = "";
                             }else if(frmSrc == "{tabContainerDyn}"){
			frmSrc = "/system/resources/services/appTabSet/switchContainerDyn.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_container";
			dataSrc = "";
                            }else if(frmSrc == "{tabControlCor}"){
			frmSrc = "/system/resources/services/appTabSet/switchControllerCor.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			dataSrc = "";
		}else if(frmSrc == "{tabControlRev}"){
			frmSrc = "/system/resources/services/appTabSet/switchControllerRev.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			dataSrc = "";
		}else if(frmSrc == "{tabContainer}"){
			frmSrc = "/system/resources/services/appTabSet/switchContainer.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_container";
			dataSrc = "";
		}else if(frmSrc == "{tabControlFront}"){
			frmSrc = "/fil/system/resources/services/appTabSet/switchController.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_control";
			dataSrc = "";
		}else if(frmSrc == "{tabContainerFront}"){
			frmSrc = "/fil/system/resources/services/appTabSet/switchContainer.xfrm";
			frmName = this.__form.__name + this.__elem.getAttribute("id")+"_container";
			dataSrc = "";	
		}else if(frmSrc == "{appForm}"){
			if(this.__model.__appFormType == "constructed"){
				frmSrc = "/system/resources/services/appForms/" + this.__model.__appForm + "Form.xfrm";
				frmConfig = this.__model.__appFormConfig; 
			}else if(this.__model.__appFormType == "FOA"){	
				appFormFOA = this.__model.__appFormFOA;
				frmSrc = "/system/resources/fundingOps/" + appFormFOA + this.__model.__appForm;
				frmConfig = "";
			}else if(this.__model.__XformSet.getRoot().___dashboard == 1 && !this.__model.__appForm){	
				//frmSrc = "/fil/home/resources/testing/blank.xfrm";
				frmSrc = "/fil/system/resources/applications/USProject/forms/actions.xfrm";
				frmConfig = "";
			}else{
				//frmSrc = "/system" + this.__model.__appForm;
				frmSrc = this.__model.__appForm;
				frmConfig = "";
			}
			frmName = this.__form.__name + this.__elem.getAttribute("id");	
		}
	var frmUri = this.__form.resolveUri(frmSrc);
		...
		if(frmConfig) this.__sfrm.__config = frmConfig;
		var params = Xml.match(this.__elem, "param");
        if (params.length > 0) {
	        var nXpCtx = {
       		 	node : xNode,
...
XFIxDialog.prototype.perform = function(xpCtx, defUp) {
    ...
    this.addEventListener("xforms-close", diag);
  	...
  	if(document.__CENTERDIALOGS || title=="Identify Field of Interest"){
  	    //SH.print("XFIxDialog: Resetting");
  	    //SH.print("rect.y' = " + rect.y);
  	    //SH.print("rect.x'= " + rect.x);
  		...
  		var off = getOffset(this.__hParent);
  		rect.y = off.top - 700;
  	    }else{
  	        rect.y = 0;
  	    };
...
XFIxDialog.__onClose  = function() {
	if (this.__xfctrl) {
	    if(this.__xfctrl.__form && this.__xfctrl.__form.__defaultModel && this.__xfctrl.__form.__defaultModel.selectNodeList("instance('form')/searchText", null).length > 0)
            this.__xfctrl.__form.__defaultModel.setValue("instance('form')/searchText", "");
