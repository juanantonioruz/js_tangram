...
function XForm(doc, name, xb /* XMLBuilder */, uri, context) {
	...
	if (!doc || !doc.documentElement) {
		alert("problem with " + name + " at " + uri + " : "); // + (doc.documentElement == null));
...
this.formCount=0;
//alert("rendering form: " + this.documentURI);
this.p = 0;
...
if(!App.myStack){ App.myStack = new Array(); }
ami = App.myStack.length;
App.myStack[ami] = new Object();
App.myStack[ami].kidsX = kids;
App.myStack[ami].xpCtxX = xpCtx;
App.myStack[ami].upX = up;
App.myStack[ami].thisX = this;
//alert(JSON.stringify(App.myStack));
//setTimeout("alert(JSON.stringify(App.myStack[" + ami + "]));", 100);
//App.myStack = new Array();
//setTimeout("alert(JSON.stringify({'b':1, 'bill':28}));", 100);

    for (var i=0; i < kids.length; i++) {
	this.__descendForm(kids[i], xpCtx, up, this, null);
//	setTimeout("App.myStack[" + ami + "].thisX.__descendForm(App.myStack[" + ami + "].kidsX[" + i + "], App.myStack[" + ami + "].xpCtxX, App.myStack[" + ami + "].upX, App.myStack[" + ami + "].thisX, null);", 100);
    }
//if(App.examine == 1){ alert("returned from exam!!!"); }
App.examine = 0;
//alert("p= out " + this.p++ + " = " + this.documentURI);
...
//	if(App.examine){if(App.count % 100 == 0){ alert("xform element" + App.count + "-" + this.formCount);} App.count++; } 
//	if(App.examine){if(App.count > 360 && App.count < 380 && App.count % 100 == 0){ alert("xform element" + App.count);} App.count++; } 
//	if(App.examine){if(App.count > 360 && App.count < 380){ alert("in doXform:" + App.count);}}
//	if(App.examine){if(App.count > 360 && App.count < 380){ alert("in doXform:" + App.count);}}
	if(App.examine){
		
//		
		var ln = xmlGetLocalName(node);
		if(ln == "repeat" && false){		
			alert(node.namespaceURI); //IDEATE2			
			alert(ln);
			var eClass = XForm.ELEMENT_CLASSES[ln];
			alert(eClass);
			alert("xform element" + App.count);
		}
	App.count++; } 

 		this.__doXformElem(node, xpCtx, uiParent, pObj, uiBefore);
//	if(App.examine){if(App.count > 360 && App.count < 380){ alert("out doXform" + App.count);}}
        } else {
	//if(App.examine){alert("pass through element");}
            var hp = uiParent;
            var pp = pObj;
            var np = null;
            ...
            var kids = node.childNodes;
            for (var i = 0; i < kids.length; i++) {
		if(App.examine && kids.length > 10) alert("length = " + kids.length);
...
XForm.prototype.__doXformElem = function(elem, xpCtx, uiParent, pObj, uiBefore) {
    ...
    var eClass = XForm.ELEMENT_CLASSES[ln];
    if (eClass != null) {
        var xe = new eClass(this, elem, pObj);	//IDEATE3
	...
	if (xpCtx.position != null) {
	    ...
	    if(!xe.isMonster || (xe.isMonster == "ie" && !SH.is_ie)){
		...
		if(!App.monsterStack){ App.monsterStack = new Array(); }
		ami = App.monsterStack.length;
		App.monsterStack[ami] = new Object();
		App.monsterStack[ami] = {monster : this, elemX : elem, hElemX : hElem, xeX : xe, xXpCtx : new Array()};

		    for (var i =0; i < ns.length; i++) {
	                nXpCtx = {
	                    node : ns[i], position : i + 1, size : ns.length,
	                    varRes : xpCtx.varRes.makeChild() };
			App.monsterStack[ami].xXpCtx[i] = nXpCtx;
			setTimeout("App.monsterStack[" + ami + "].monster.__doXformBody(App.monsterStack[" + ami + "].elemX, App.monsterStack[" + ami + "].xXpCtx[" + i + "], App.monsterStack[" + ami + "].hElemX, App.monsterStack[" + ami + "].xeX, null, true);"
				+ (i == ns.length - 1 ? "App.monsterStack[" + ami + "].xeX.__endRender();App.monsterStack[" + ami + "]=null;" : ""), 0);
	            }
	    }
...
XFModel.prototype.getValue = function(xpath, contextNode) {
    if(!this.__defaultInstance && !contextNode) return null;
    var xpCtx = {
...
XFModel.prototype.setValue = function(xpath, value, contextNode) {
    if(!contextNode && (!this.__defaultInstance || !this.__defaultInstance.__root)) return; //IDEATE
    var xpCtx = {
        node : contextNode || this.__defaultInstance.__root,
        varRes : this.__form.__xpVarRes};
...
XFModel.prototype.activateCase = function(id) {
    if(!this.__form || !this.__form.__cases){return;};
